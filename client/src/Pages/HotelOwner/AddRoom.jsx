...
const AddRoom = () => {
   ...
   const onSubmitHandler = async(e) => {
    ...
    try {
        ...
        // ✅ Fixed: added leading slash and parentheses to getToken()
        const {data} = await axios.post('/api/rooms', formData, {
            headers: {
                Authorization: `Bearer ${await getToken()}` // ✅ Added ()
            }
        })
            ...
            <div className="grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap">
                {Object.keys(images).map((key) => (
                    <label htmlFor={`roomImage${key}`} key={key}>
                        <img
                            className="h-12 cursor-pointer opacity-80"
                            src={images[key] ? URL.createObjectURL(images[key]) : assets.uploadArea}
                            alt="Room Image Upload"
                        />
                        <input
                            type="file"
                            accept="image/*"
    ...
    disabled={loading}
>
    {loading ? 'Adding...' : 'Add Room'} {/* ✅ Fixed typo */}
</button>
